...
rain <- read_parquet("shiny/climate/rain.parquet")
temp <- read_parquet("shiny/climate/temp.parquet")
#-------------------------------------------
...
temp %>%
  filter(month %in% c(6), elev < 1200, status == "official") %>%
...
temp %>% 
  ...
  labs(x = "Ден", y = "Средна денонощна температура (\u00B0C)",
       ...
       subtitle = "Месец:", caption = "Източник на данните: www.stringmeteo.com") +
