...
mean_temp_month <- temp %>%
	drop_na() %>% 
	filter(month %in% c(6, 7, 8), elev < 1200, day %in% c(1:31)) %>%
	group_by(year) %>% 
...
temp %>%
  ...
  geom_boxplot(aes(fill = col), fatten = NULL) +
	...
	labs(x = "Години", y = "Средна месечна температура (ºС)", fill = "Легенда:", title = "Месец: Юли") +
...
mean_rain_month <- rain %>% 
  drop_na() %>% 
  filter(month %in% c(7), elev < 1200, day %in% c(1:14)) %>%
...
rain %>%
  ...
  labs(x = "Години", y = "Средно месечно количество на валежите (mm)", fill = "Легенда:", title = "Месец: Юли") +
  theme(text = element_text(size = 14), legend.position = "right") +
